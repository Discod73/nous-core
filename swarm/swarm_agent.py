"""
NOUS Swarm Agent — P2P knowledge sharing (Fase 3: Kin + credits + wing-config).
Port 8020
"""
import hashlib
import json
...
try:
    ...
    logging.warning("datasketch ikke tilgængeligt — falder tilbage til SHA256-only dedup")
...
# MinHash LSH config
_LSH_NUM_PERM  = 128
_LSH_THRESHOLD = 0.8   # Jaccard-similarity: facts med overlap >= 80% er near-dupes
...
NX_TOTAL_VRAM_MB   = int(os.environ.get("NX_TOTAL_VRAM_MB",     "8192"))

# Scope der ALDRIG må forwvardes til peer-compute
_FORBIDDEN_FORWARD_SCOPES = {"SECRET", "PRIVATE"}
...
app = FastAPI(title="NOUS Swarm Agent", version="3.1", lifespan=lifespan)
...
class InferRequest(BaseModel):
    prompt: str
    max_tokens: int = 512
    scope: str = "SWARM"  # SECRET/PRIVATE afvises med 403
...
def _fact_exists_in_incoming(text_hash: str) -> bool:
    """Lag 1: eksakt SHA256-match i swarm_incoming."""
    try:
        r = httpx.post(
...
# ── MinHash LSH (Lag 2: near-duplicate detection) ────────────────────────────
...
def _text_to_minhash(text: str) -> "MinHash":
    """Konvertér tekst til MinHash via karakter-4-grams.

    Karakter-shingles giver bedre Jaccard-overlap for korte fakta-tekster
    (typisk 50-200 tegn) end ord-n-grams, og fanger bl.a. ord-bytning og
    stavevarianter. For tekster kortere end 4 tegn bruges hele teksten.
    """
    ...
    if len(t) < k:
        ...
        for i in range(len(t) - k + 1):
            m.update(t[i:i + k].encode("utf-8"))
    return m


def _populate_lsh_from_qdrant(index: "MinHashLSH") -> int:
    """Fyld LSH-index med eksisterende facts fra swarm_incoming. Returnerer antal indlæste."""
    count = 0
    offset = None
    while True:
        ...
        for pt in result.get("points", []):
            ...
            if text and key not in index:
                try:
                    index.insert(key, _text_to_minhash(text))
                    count += 1
                except Exception:
                    pass
...
def _get_lsh_index() -> "MinHashLSH":
    """Returnér LSH-index; populér lazily fra Qdrant ved første kald."""
    global _lsh_index
    with _lsh_lock:
        if _lsh_index is None:
            idx = MinHashLSH(threshold=_LSH_THRESHOLD, num_perm=_LSH_NUM_PERM)
            n   = _populate_lsh_from_qdrant(idx)
            _lsh_index = idx
            log.info(f"MinHash LSH index initialiseret med {n} eksisterende facts (threshold={_LSH_THRESHOLD})")
    return _lsh_index


def _lsh_insert(point_id: str, text: str) -> None:
    """Tilføj et fact til LSH-index (kaldes efter vellykket write til Qdrant)."""
    if not _MINHASH_AVAILABLE:
        return
    try:
        idx = _get_lsh_index()
        with _lsh_lock:
            if point_id not in idx:
                idx.insert(point_id, _text_to_minhash(text))
    except Exception as e:
        log.warning(f"LSH insert fejl for {point_id}: {e}")


def _is_near_duplicate(text: str, sha_hash: str) -> bool:
    """
    To-lags dedup:
      Lag 1 — SHA256 eksakt match (hurtigt, via Qdrant payload-filter)
      Lag 2 — MinHash LSH near-duplicate, Jaccard >= 0.8 (fanger semantisk nær-identiske)
    Returnerer True hvis fact allerede eksisterer eller er near-duplikat.
    """
    # Lag 1: eksakt SHA256
    if _fact_exists_in_incoming(sha_hash):
        return True
    ...
    try:
        ...
        neighbors = idx.query(mh)
        if neighbors:
            log.debug(f"MinHash near-dupe fundet (neighbors={neighbors[:3]}, text={text[:60]!r})")
            return True
    except Exception as e:
        log.warning(f"MinHash near-dupe check fejl: {e}")
    return False
...
async def _write_to_incoming(fact: PeerIngestFact, source_label: str) -> None:
    ...
    # Registrér i LSH-index så fremtidige near-dupe checks er opdaterede
    _lsh_insert(point_id, fact.text)
...
def _get_facts_from_wings(allowed_wings: list[str]) -> list[dict]:
    """Facts direkte fra private wings — til Kin krypteret sync."""
    node_id = get_node_id()
    wmap = _wing_map()
    facts = []
...
async def _sync_plain(peer: dict, our_facts: list[dict]) -> dict:
    """Standard ukrypteret sync (global/work peers)."""
    entry: dict = {
    ...
    try:
        ...
        for f in peer_facts:
            ...
            try:
                ...
                log.warning(f"Write fejl (Kin fact fra {peer['label']}): {e}")
        entry["received"] = new_count
        ...
        update_last_seen(peer["node_id"])
        log.info(f"Kin sync {peer['label']} ({group['name']}): {new_count} modtaget, {len(encrypted_facts)} sendt")
    except Exception as e:
        entry["error"] = str(e)
        log.warning(f"Kin sync fejl ({peer['label']}): {e}")
    return entry
...
def _ollama_ps() -> dict:
    """Hent models_loaded og gpu_free_mb fra Ollama /api/ps."""
    try:
        r = httpx.get(f"{OLLAMA_URL}/api/ps", timeout=5.0)
        r.raise_for_status()
        models = r.json().get("models", [])
        used_mb = sum(m.get("size_vram", 0) for m in models) / 1024 / 1024
        return {
            "models_loaded": [m["name"] for m in models],
            "gpu_free_mb":   max(0, round(NX_TOTAL_VRAM_MB - used_mb)),
        }
    except Exception:
        return {"models_loaded": [], "gpu_free_mb": -1}
...
def health():
    ...
    with _active_inference_lock:
        busy = _active_inference_count > 0
    gpu    = _ollama_ps()
    return {
        "node_id":           get_node_id(),
        "version":           "3.1",
        ...
        "dedup": {
            "minhash_available": _MINHASH_AVAILABLE,
            "threshold":         _LSH_THRESHOLD,
            "num_perm":          _LSH_NUM_PERM,
            "lsh_index_ready":   _lsh_index is not None,
        },
    }
...
def get_outgoing_facts(group_id: str = Query(None)):
    ...
    group_id: returnér krypterede Kin-facts fra allowed_wings.
...
async def ingest_from_peer(body: PeerIngestRequest, request: Request):
    ...
    for fact in body.facts:
        ...
        if _is_near_duplicate(fact_text, h):
            continue
        try:
            await _write_to_incoming(
                PeerIngestFact(
...
async def sync_with_peers():
    """Sync med alle aktive peers. Kin-peers: krypteret PSK-sync."""
...
def api_peers_status():
    """Returnér alle peers med live load-status fra PeerLoadCache (30s TTL)."""
    peers = get_all_peers()
    result = []
    for p in peers:
        health = peer_load_cache.get(p, timeout=4.0)
        entry = {
            "node_id":      p["node_id"],
            "label":        p["label"],
            "tailscale_ip": p["tailscale_ip"],
            "port":         p["port"],
            "swarm_type":   p["swarm_type"],
            "last_seen":    p.get("last_seen"),
        }
        if health:
            entry["online"]        = True
            entry["status"]        = health.get("status", "unknown")
            entry["gpu_free_mb"]   = health.get("gpu_free_mb", -1)
            entry["models_loaded"] = health.get("models_loaded", [])
            entry["nat_hours"]     = health.get("nat_hours", False)
            entry["compute"]       = health.get("offerings", {}).get("compute", False)
        else:
            entry["online"]        = False
            entry["status"]        = "offline"
            entry["gpu_free_mb"]   = -1
            entry["models_loaded"] = []
            entry["nat_hours"]     = False
            entry["compute"]       = False
        result.append(entry)
    return {"peers": result}


def find_idle_peer_for_compute() -> dict | None:
    """Vælg bedste idle peer til compute-delegation (højeste credit-prioritet)."""
    candidates = []
    for p in get_active_peers():
        health = peer_load_cache.get(p, timeout=4.0)
        if not health:
            continue
        if health.get("status") != "idle":
            continue
        if not health.get("nat_hours", False):
            continue
        if not health.get("offerings", {}).get("compute", False):
            continue
        priority = get_priority_for_peer(p["node_id"])
...
async def swarm_infer(body: InferRequest):
    if body.scope.upper() in _FORBIDDEN_FORWARD_SCOPES:
        raise HTTPException(403, f"Scope '{body.scope}' må ikke videresendes til peer-compute — brug kun SWARM-scope data")
