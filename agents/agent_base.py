...
_MODEL_ROLES_FILE = Path("/mnt/nous-data/model_roles.json")
...
def load_role_params(role: str = "day") -> dict:
    """Hent gemte model-parametre for 'day' eller 'night' fra model_roles.json."""
    try:
        if _MODEL_ROLES_FILE.exists():
            data = json.loads(_MODEL_ROLES_FILE.read_text(encoding="utf-8"))
            return {**_DEFAULT_PARAMS, **data.get(f"{role}_params", {})}
    except Exception:
        pass
